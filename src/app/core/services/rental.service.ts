...
export class RentalService {
    ...
    sendRequest(form: RequestSend): Observable<boolean | void> {
        // console.log(form)
        console.log(form);
        return this.http.post<RequestSend>(this.API, form).pipe(
            map((res) => {
    ...
    cancelRequestByIdRequest(requestId: string): Observable<boolean | void> {
        return this.http.put<idRequest>(`${this.API}/cancel`, { requestId }).pipe(
            map((res: any) => {
                if (res) return true;
                else return false;
            }),
    ...
    confirmRequestByIdRequest(requestId: string): Observable<boolean | void> {
        ...
        return this.http.put<idRequest>(`${this.API}/confirm`, { requestId }).pipe(
            map((res: any) => {
                // this.saveToken(res.token)
                if (res) return true;
                else return false;
